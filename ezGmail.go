...
func tokenCacheFile() (string, error) {
	...
	tokenCacheDirErr := os.MkdirAll(tokenCacheDir, 0700)
	return filepath.Join(tokenCacheDir, url.QueryEscape("ezGmail.json")), tokenCacheDirErr
}
...
func (gs *GmailService) InitSrv() {
	...
	gs.srv, err = gmail.NewService(context.Background(), option.WithHTTPClient(client))
	if err != nil {
...
func (gm *GmailMessage) parseMessagePart(origmsg *gmail.MessagePart, gs *GmailService) {
	...
	for _, ii := range origmsg.Headers {
		if ii.Name == "Subject" {
			gm.sSubject = ii.Value
		}
		if ii.Name == "Content-Disposition" {
			contentDisp = ii.Value
		}
		if ii.Name == "Message-ID" {
			gm.sMessageId = ii.Value
		}
	}
...
func (gs *GmailService) GetListOnly() *gmail.ListMessagesResponse {
	...
	if len(gs.sLabel) > 0 {
		gsCall = gsCall.LabelIds(gs.sLabel)
	}
	...
	if len(gs.sOlder) > 0 {
		qStr += "older:" + gs.sOlder + " "
	}
	if len(gs.sNewer) > 0 {
		qStr += "newer:" + gs.sNewer + " "
	}
	if len(gs.sOlderRel) > 0 {
		qStr += "older_than:" + gs.sOlderRel + " "
	}
	if len(gs.sNewerRel) > 0 {
		qStr += "newer_than:" + gs.sNewerRel + " "
	}
	if len(gs.sSubject) > 0 {
		qStr += "subject:\"" + gs.sSubject + "\" "
	}
	if len(gs.sInPlace) > 0 {
		qStr += "in:" + gs.sInPlace + " "
	}
	if len(gs.sLarger) > 0 {
		qStr += "larger:" + gs.sLarger + " "
	}
	if len(gs.sSmaller) > 0 {
		qStr += "smaller:" + gs.sSmaller + " "
	}
	if len(gs.sFilename) > 0 {
		qStr += "filename:\"" + gs.sFilename + "\" "
	}
	if len(gs.sMatch) > 0 {
		qStr += " " + gs.sMatch + " "
	}
	if len(gs.sMatchExact) > 0 {
		qStr += " " + gs.sMatchExact + " "
	}
	if gs.sHasAttachment {
		qStr += "has:attachment "
	}
	if len(qStr) > 0 {
